import { IFreezedSaveService } from '../IFreezedSaveService';

class DetailsParams {
    public item: any;
}

interface IFreezedPanelBindings {
    [key: string]: any;
...
const FreezedPanelBindings: IFreezedPanelBindings = {
    item: '<?iqsObject',
    onDetails: '&iqsOpenDetails'
}

class FreezedPanelChanges implements ng.IOnChangesObject, IFreezedPanelBindings {
...
class FreezedPanelController implements IFreezedPanelBindings {
    public item: any;
    ...
    public statType: string = 'freezed';
    ...
    constructor(
        ...
        private iqsFreezedSaveService: IFreezedSaveService,
        ...
        $element.addClass('iqs-freezed-panel');
        this.restore();
    ...
    private restore(): void {
        if (this.$location.search()['view'] == iqs.shell.StatisticsView.Chart || this.$location.search()['view'] == iqs.shell.StatisticsView.Grid) {
            ...
            this.view = this.iqsFreezedSaveService.view ? this.iqsFreezedSaveService.view : iqs.shell.StatisticsView.Chart;
            this.$location.search('view', this.view);
            this.iqsFreezedSaveService.view = this.view;
        }
        ...
        let zoneId;
        if (!this.$location.search()['zone_stat_id'] && this.iqsFreezedSaveService.filterParams && this.iqsFreezedSaveService.filterParams.zoneId) {
            zoneId = this.iqsFreezedSaveService.filterParams.zoneId;
        } else {
    ...
    public generateStatsDescription(secondArg: string, thirdArg: string, each: boolean = false) {
        // this.statsDescription = 'STATS_FREEZED_ALL';
        this.gridValueLabel = 'STATISTICS_FREEZED_VALUE';
        this.gridNameLabel = 'STATISTICS_FREEZED_TIME';

        if (!this.filterParams.objectId && !this.filterParams.objectGroupId) {
            this.statsDescription = 'STATS_FREEZED_ALL_OBJECTS';
            this.statsDescriptionDetails = null;
        } else if (this.filterParams.objectGroupId) {
            this.statsDescription = 'STATS_FREEZED_GROUP_OBJECTS';
            ...
            this.statsDescription = 'STATS_FREEZED_OBJECT';
        ...
        if (secondArg == 'all' && thirdArg == 'all') {
            // this.statsDescription = 'STATS_FREEZED_ALL';
            this.gridNameLabel = 'STATISTICS_FREEZED_ZONES';
            // this.gridNameLabel = 'STATISTICS_FREEZED_OBJECT_AND_GROUPS';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg == 'all' && !each) {
            // this.statsDescription = 'STATS_FREEZED_BY_OBJECT_EACH_ZONE';
            // this.gridNameLabel = 'STATISTICS_FREEZED_ZONES';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg == 'all' && each) {
            // this.statsDescription = 'STATS_FREEZED_EACH_OBJECT_OF_GROUP';
            // this.gridNameLabel = 'STATISTICS_FREEZED_OBJECT_OF_GROUPS';
            // this.gridType = GridTypes.Group;
            return;
        }
        if (secondArg == 'all' && thirdArg != 'all') {
            // this.statsDescription = 'STATS_FREEZED_EACH_OBJECT_ZONE';
            // this.gridNameLabel = 'STATISTICS_FREEZED_OBJECT_AND_GROUPS';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg != 'all' && !each) {
            // this.statsDescription = 'STATS_FREEZED_BY_OBJECT_BY_ZONE';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg != 'all' && each) {
            // this.statsDescription = 'STATS_FREEZED_EACH_OBJECT_OF_GROUP_BY_ZONE';
            // this.gridNameLabel = 'STATISTICS_FREEZED_OBJECT_OF_GROUPS';
            // this.gridType = GridTypes.Group;
            return;
        }
    ...
    private setUrlParams(params) {
        ...
        this.iqsFreezedSaveService.filterParams = params;
    }
    ...
    public $onChanges(changes: FreezedPanelChanges): void {
        this.prepare();
    }
...
(() => {
    angular
        .module('iqsFreezedPanel', [
            'iqsStatistics',
            'iqsObjectGroups.ViewModel',
            'iqsObjects.ViewModel',
        ])
        .component('iqsFreezedPanel', {
            bindings: FreezedPanelBindings,
            templateUrl: 'statistics/freezed/panels/FreezedPanel.html',
            controller: FreezedPanelController,
            controllerAs: '$ctrl'
        })
})();
