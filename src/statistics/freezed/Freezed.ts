...
export const FreezedStateName: string = 'app.freezed';

class FreezedController implements ng.IController {
    public $onInit() { }
    private cf: Function[] = [];
    ...
    constructor(
        ...
        const runWhenReady = () => {
            ...
            this.selectedIndex = Math.max(0, _.findIndex(this.collection,
                (param) => { return param.id == this.iqsFreezedSaveService.objectId; }
            ));
            this.selectItem();
    ...
    private saveCurrentState() {
        this.iqsFreezedSaveService.objectId = this.collection && this.collection.length > 0 && this.selectedIndex > -1 ? this.collection[this.selectedIndex].id : null;
        this.iqsFreezedSaveService.details = this.details;
    }

    private restoreState() {
        if (!this.$location.search()['details']) {
            this.$location.search('details', this.iqsFreezedSaveService.details);
            this.details = this.iqsFreezedSaveService.details;
        } else {
            this.details = this.$location.search()['details']
        }
        if (!this.$location.search()['object_stat_id'] && this.iqsFreezedSaveService.objectId) {
            this.$location.search('object_stat_id', this.iqsFreezedSaveService.objectId);
        } else {
            if (this.$location.search()['object_stat_id']) {
                this.iqsFreezedSaveService.objectId = this.$location.search()['object_stat_id'];
            }
        }
    }
        ...
        if (!this.pipMedia('gt-sm') && this.details) {
            this.pipNavService.breadcrumb.items = [
                <pip.nav.BreadcrumbItem>{
                    title: "STATISTICS_FREEZED_TITLE", click: () => {
                        this.toMainFromDetails();
                    }, subActions: []
                },
                <pip.nav.BreadcrumbItem>{
                    title: this.pipTranslate.translate('STATISTICS_FREEZED_DETAILS_TITLE') + this.getSelectedName(), click: () => { }, subActions: []
                }
            ];
            ...
            this.pipNavService.breadcrumb.text = 'STATISTICS_FREEZED_TITLE';
...
function configureFreezedRoute(
    ...
    $stateProvider
        .state(FreezedStateName, {
            url: '/freezed?details&view&type&date&to_date&object_stat_id&zone_stat_id',
            controller: FreezedController,
            reloadOnSearch: false,
            auth: true,
            controllerAs: '$ctrl',
            templateUrl: 'statistics/freezed/Freezed.html'
        });
}

function configureFreezedAccess(
    ...
    iqsAccessConfigProvider.registerStateAccess(FreezedStateName, accessLevel);

    iqsAccessConfigProvider.registerStateConfigure(FreezedStateName, accessConfig);
}
...
(() => {
    ...
    angular
        .module('iqsNewStatistics.Freezed', [
            'pipNav',
            'iqsFreezed.SaveService',
            'iqsFreezedEmptyPanel',
            'iqsFreezedPanel'
        ])
        .config(configureFreezedRoute)
        .config(configureFreezedAccess);
})();
