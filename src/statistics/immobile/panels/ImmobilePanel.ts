import { IImmobileSaveService } from '../IImmobileSaveService';

class DetailsParams {
    public item: any;
}

interface IImmobilePanelBindings {
    [key: string]: any;
...
const ImmobilePanelBindings: IImmobilePanelBindings = {
    item: '<?iqsObject',
    onDetails: '&iqsOpenDetails'
}

class ImmobilePanelChanges implements ng.IOnChangesObject, IImmobilePanelBindings {
...
class ImmobilePanelController implements IImmobilePanelBindings {
    public item: any;
    ...
    public statType: string = 'immobile';
    ...
    constructor(
        ...
        private iqsImmobileSaveService: IImmobileSaveService,
        ...
        $element.addClass('iqs-immobile-panel');
        this.restore();
    ...
    private restore(): void {
        if (this.$location.search()['view'] == iqs.shell.StatisticsView.Chart || this.$location.search()['view'] == iqs.shell.StatisticsView.Grid) {
            ...
            this.view = this.iqsImmobileSaveService.view ? this.iqsImmobileSaveService.view : iqs.shell.StatisticsView.Chart;
            this.$location.search('view', this.view);
            this.iqsImmobileSaveService.view = this.view;
        }
        ...
        let zoneId;
        if (!this.$location.search()['zone_stat_id'] && this.iqsImmobileSaveService.filterParams && this.iqsImmobileSaveService.filterParams.zoneId) {
            zoneId = this.iqsImmobileSaveService.filterParams.zoneId;
        } else {
    ...
    public generateStatsDescription(secondArg: string, thirdArg: string, each: boolean = false) {
        // this.statsDescription = 'STATS_IMMOBILE_ALL';
        this.gridValueLabel = 'STATISTICS_IMMOBILE_VALUE';
        this.gridNameLabel = 'STATISTICS_IMMOBILE_TIME';

        if (!this.filterParams.objectId && !this.filterParams.objectGroupId) {
            this.statsDescription = 'STATS_IMMOBILE_ALL_OBJECTS';
            this.statsDescriptionDetails = null;
        } else if (this.filterParams.objectGroupId) {
            this.statsDescription = 'STATS_IMMOBILE_GROUP_OBJECTS';
            ...
            this.statsDescription = 'STATS_IMMOBILE_OBJECT';
        ...
        if (secondArg == 'all' && thirdArg == 'all') {
            // this.statsDescription = 'STATS_IMMOBILE_ALL';
            this.gridNameLabel = 'STATISTICS_IMMOBILE_ZONES';
            // this.gridNameLabel = 'STATISTICS_IMMOBILE_OBJECT_AND_GROUPS';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg == 'all' && !each) {
            // this.statsDescription = 'STATS_IMMOBILE_BY_OBJECT_EACH_ZONE';
            // this.gridNameLabel = 'STATISTICS_IMMOBILE_ZONES';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg == 'all' && each) {
            // this.statsDescription = 'STATS_IMMOBILE_EACH_OBJECT_OF_GROUP';
            // this.gridNameLabel = 'STATISTICS_IMMOBILE_OBJECT_OF_GROUPS';
            // this.gridType = GridTypes.Group;
            return;
        }
        if (secondArg == 'all' && thirdArg != 'all') {
            // this.statsDescription = 'STATS_IMMOBILE_EACH_OBJECT_ZONE';
            // this.gridNameLabel = 'STATISTICS_IMMOBILE_OBJECT_AND_GROUPS';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg != 'all' && !each) {
            // this.statsDescription = 'STATS_IMMOBILE_BY_OBJECT_BY_ZONE';
            // this.gridType = GridTypes.Formated;
            return;
        }
        if (secondArg != 'all' && thirdArg != 'all' && each) {
            // this.statsDescription = 'STATS_IMMOBILE_EACH_OBJECT_OF_GROUP_BY_ZONE';
            // this.gridNameLabel = 'STATISTICS_IMMOBILE_OBJECT_OF_GROUPS';
            // this.gridType = GridTypes.Group;
            return;
        }
    ...
    private setUrlParams(params) {
        ...
        this.iqsImmobileSaveService.filterParams = params;
    }
    ...
    public $onChanges(changes: ImmobilePanelChanges): void {
        this.prepare();
    }
...
(() => {
    angular
        .module('iqsImmobilePanel', [
            'iqsStatistics',
            'iqsObjectGroups.ViewModel',
            'iqsObjects.ViewModel',
        ])
        .component('iqsImmobilePanel', {
            bindings: ImmobilePanelBindings,
            templateUrl: 'statistics/immobile/panels/ImmobilePanel.html',
            controller: ImmobilePanelController,
            controllerAs: '$ctrl'
        })
})();
