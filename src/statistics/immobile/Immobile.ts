...
export const ImmobileStateName: string = 'app.immobile';

class ImmobileController implements ng.IController {
    public $onInit() { }
    private cf: Function[] = [];
    ...
    constructor(
        ...
        const runWhenReady = () => {
            ...
            this.selectedIndex = Math.max(0, _.findIndex(this.collection,
                (param) => { return param.id == this.iqsImmobileSaveService.objectId; }
            ));
            this.selectItem();
    ...
    private saveCurrentState() {
        this.iqsImmobileSaveService.objectId = this.collection && this.collection.length > 0 && this.selectedIndex > -1 ? this.collection[this.selectedIndex].id : null;
        this.iqsImmobileSaveService.details = this.details;
    }

    private restoreState() {
        if (!this.$location.search()['details']) {
            this.$location.search('details', this.iqsImmobileSaveService.details);
            this.details = this.iqsImmobileSaveService.details;
        } else {
            this.details = this.$location.search()['details']
        }
        if (!this.$location.search()['object_stat_id'] && this.iqsImmobileSaveService.objectId) {
            this.$location.search('object_stat_id', this.iqsImmobileSaveService.objectId);
        } else {
            if (this.$location.search()['object_stat_id']) {
                this.iqsImmobileSaveService.objectId = this.$location.search()['object_stat_id'];
            }
        }
    }
        ...
        if (!this.pipMedia('gt-sm') && this.details) {
            this.pipNavService.breadcrumb.items = [
                <pip.nav.BreadcrumbItem>{
                    title: "STATISTICS_IMMOBILE_TITLE", click: () => {
                        this.toMainFromDetails();
                    }, subActions: []
                },
                <pip.nav.BreadcrumbItem>{
                    title: this.pipTranslate.translate('STATISTICS_IMMOBILE_DETAILS_TITLE') + this.getSelectedName(), click: () => { }, subActions: []
                }
            ];
            ...
            this.pipNavService.breadcrumb.text = 'STATISTICS_IMMOBILE_TITLE';
...
function configureImmobileRoute(
    ...
    $stateProvider
        .state(ImmobileStateName, {
            url: '/immobile?details&view&type&date&to_date&object_stat_id&zone_stat_id',
            controller: ImmobileController,
            reloadOnSearch: false,
            auth: true,
            controllerAs: '$ctrl',
            templateUrl: 'statistics/immobile/Immobile.html'
        });
}

function configureImmobileAccess(
    ...
    iqsAccessConfigProvider.registerStateAccess(ImmobileStateName, accessLevel);

    iqsAccessConfigProvider.registerStateConfigure(ImmobileStateName, accessConfig);
}
...
(() => {
    ...
    angular
        .module('iqsNewStatistics.Immobile', [
            'pipNav',
            'iqsImmobile.SaveService',
            'iqsImmobileEmptyPanel',
            'iqsImmobilePanel'
        ])
        .config(configureImmobileRoute)
        .config(configureImmobileAccess);
})();
